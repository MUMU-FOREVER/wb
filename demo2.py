...
def check_and_sync():
    ...
    if latest_weibo:
        created_at = datetime.fromtimestamp(latest_weibo.get("created_at"))
        if (datetime.now() - created_at).total_seconds() < 3600:  # 如果微博是在过去一小时内发布的
            message = f"新微博消息：\n{latest_weibo.get('text_raw', '无内容')}\n发布时间：{created_at}"
...
def main():
    ...
    latest_weibo = get_latest_weibo()
    print(latest_weibo)
    long_text = get_long_text(latest_weibo.get("mblogid"));
    message = f"新微博消息：\n{long_text}\n发布时间：{parse_weibo_time(latest_weibo.get('created_at'))}"
